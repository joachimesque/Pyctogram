...
def get_user_data(user_name):
    ...
    try:
        ...
        print("Error: Something happened with the connexion")
        db.stop_db()
...
def user_exists(user):
    ...
    db.cursor.execute("SELECT count(*) FROM Accounts WHERE username = ?", (user,))
...
def add_new_user(user_data):
    ...
    try:
        db.cursor.execute("INSERT INTO Accounts VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", values)
        return True
    except:
...
def get_user_timestamp(user_id):
    ...
    try:
        db.cursor.execute("SELECT last_updated FROM Accounts WHERE id=?", (user_id,))
        return(db.cursor.fetchone())
    except:
        exit("Error: Getting the timestamp of %s has failed." % user_id)


def set_user_timestamp(user_id, timestamp):
    # Sets the timestamp (latest upload we have in DB) of an user

    try:
        db.cursor.execute("UPDATE Accounts SET last_updated=? WHERE id=?", (timestamp,user_id,))
    except:
...
def add_data_to_db(user_data):
    ...
    for media in user_data['edge_owner_to_timeline_media']['edges']:
        ...
        if node['taken_at_timestamp'] > user_timestamp and node['__typename'] != 'GraphVideo':
            ...
            try:
                db.cursor.execute("INSERT INTO Media VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", values)
                media_count += 1
            except:
                exit("Error: Adding an image by %s into the database failed." % user_data["username"])


    set_user_timestamp(user_data['id'], latest_timestamp)

    if media_count > 0:
        print("%d new media added for user %s" % (media_count, user_data['username']))

def tag_user_as_deleted(user):
    # Tags user as DELETED
    try:
        db.cursor.execute("UPDATE Accounts SET is_deleted=1 WHERE username=?", (user,))
    except:
        exit("Error: Changing the status of user %s failed." % user)

def tag_user_as_existing(user):
    # Removes DELETED tag
    try:
        db.cursor.execute("UPDATE Accounts SET is_deleted=0 WHERE username=?", (user,))
    except:
        exit("Error: Changing the status of user %s failed." % user)
